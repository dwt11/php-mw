...
{
    MkdirAll($cfg_basedir.$activepath."/$mdir",$cfg_dir_web_role);
    CloseFtp();
}
...
{
    //分析远程文件路径
    $remotefile = str_replace(DEDEPATH, '', $fullfilename);
...
$inquery = "INSERT INTO `#@__uploads`(logid,title,url,mediatype,width,height,playtime,filesize,uptime,mid)
...
ShowMsg("成功上传一幅图片！","select_images.php?imgstick=$imgstick&noeditor=yes&comeback=".urlencode($filename_name)."&v=$v&f=$f&activepath=".urlencode($activepath)."/$mdir&d=".time());
exit();