 ...
 *   2010-11-17 [添加]添加检索条件匹配方法
 *   2010-11-16 [添加]如果表字段存在注释,则列表项的title中则直接显示注释信息
...
class Lurd extends DataListCP
{
    ...
    function __construct($tablename, $templatedir='', $lurdtempdir='')
    {
        ...
        if(!empty($GLOBALS['ct']))
        {
            $this->SetParameter('ct', $GLOBALS['ct']);
        }
    ...
    //强制指定字模板（列表list和编辑edit、添加add模板里用）
    ...
    function MakeListTemplate($listfield = '')
    {
        ...
        if(!empty($listfield) && $listfield != '*' )
        {
            ...
            foreach($listfields as $k)
            {
                ...
                $v = $this->fields[$k];
                $title = isset($v['comment'])? $v['comment'] : $k;
    ...
    function SaveEditData($isturn=TRUE)
    {
        ...
        {
            //自动递增键不处理
            $GLOBALS[$k] = isset($GLOBALS[$k])? $GLOBALS[$k] : request::$forms[$k];
    ...
    */
    ...
    function GetData($fname)
    {
        ...
        $GLOBALS[$fname] = isset($GLOBALS[$fname])? $GLOBALS[$fname] : @request::$forms[$fname];
        //二进制单独处理
