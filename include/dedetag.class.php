...
class DedeTagParse
{
    ...
    function LoadCache($filename)
    {
        ...
        $cachedir = DEDEPATH.$cfg_tplcache_dir;
    ...
    function Check($path, $ds = DIRECTORY_SEPARATOR) 
    {
        ...
        if (strpos($path, $this->clean(DEDEPATH)) !== 0) {
           // die('Error:check Snooping out of bounds @ '.$path);
            die('Error:请检查模板路径地址 @ '.$path);
        }
        return $path;
    }
...
//dump("------".$filename);
    ...
    {
        $fp = @fopen($filename,"w") or die("DedeTag 创建文件失败:  ".$filename);
