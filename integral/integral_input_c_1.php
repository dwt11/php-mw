...
if($dopost=='input')
{
    ...
    while($row = $dsql->GetArray('c'))
    {
				...
				$integral_bz="考勤违规C分扣分项".$row['kq_hw_emptime'].GetSm($row['kq_zt']);
				$integral_markdate=date("Y-m-d", time());
				$questr="SELECT * FROM `dede_integral` where integral_class='c' and integral_date ='".$integral_date."' and integral_empid=".$integral_empid;
  ...
  ShowMsg("考勤记录导入月份: ".$inputdate .",成功  ".$truenumb." 条,因存在重复记录失败 ".$falsenumb." 条！","integral_input_c.php");
   exit();
}
...
$tplfile = "integral_input_c_1.htm";
...
function getconfig()
 {  
  ...
  $arcQuery = "SELECT *  from #@__integral_checkinConfig  WHERE id='3' ";
  // //dump($arcQuery);
